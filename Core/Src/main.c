...
int main(void)
{
  ...
  HX711_Init(&loadcell, GPIOC, GPIO_PIN_4, GPIOC, GPIO_PIN_5);
  HX711_Tare(&loadcell, 10);  // hiệu chuẩn không tải
  HX711_SetScale(&loadcell, 100.0f); // giá trị thử, bạn sẽ cần hiệu chỉnh thật
  ...
  {
    /* USER CODE END WHILE */
	  float weight = HX711_GetUnits(&loadcell, 5);
	  printf("Force: %.2f\r\n", weight);  // hoặc truyền về UART để debug
	  HAL_Delay(100);
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : SPI5_NCS_Pin PC5 */
  GPIO_InitStruct.Pin = SPI5_NCS_Pin|GPIO_PIN_5;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
